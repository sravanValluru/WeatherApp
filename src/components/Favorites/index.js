      ...
      <div className={` ${!isVisible ? "hidden" : ""}`}>
        {favorites.length > 0 &&
          favorites.map((city) => (
            <div
              key={city}
              className="cursor-pointer bg-white hover:bg-slate-300 even:bg-gray-100 transition text-slate-900 mt-3 p-2 text-center shadow sm:mt-0 sm:ml-3 sm:flex-shrink-0"
